...
fun InquiryRoute() {
    ...
    var isChecked by rememberSaveable { mutableStateOf(List(inquiryFilterList.size) { false }) }
    ...
    InquiryScreen(
        ...
        onFilterClicked = { index ->
            isChecked = isChecked.toMutableList().also { it[index] = !it[index] }
        }
    )
}
...
fun InquiryScreen(
    ...
    BitgoeulAndroidTheme { colors, typography ->
        ...
        FilterDialog(
            ...
            onFilterClicked = onFilterClicked // 추후 클릭 이벤트 처리
        )
    }
}
